...
function cmd_profadd() {
	...
	echo -e "${BLUE}$prof_name${NC} profile successfully added, use ${GREEN}dupnode install $prof_name${NC} to create a new instance of the masternode"
...
function cmd_install() {
	...
	echo_json "{\"message\":\"dupe successfully installed\",\"ip\":\"$([[ $show_ip ]] && echo $show_ip || echo null)\",\"port\":\"$([[ $mn_port ]] && echo $mn_port || echo null)\",\"rpc\":\"$NEW_RPC\",\"dup\":$1,\"retcode\":$retcode}"
}
...
function cmd_help() {
	echo -e "Options:\
			\n  - ${YELLOW}dupnode profadd <prof_file> [prof_name]          ${NC}Adds a profile that will be used to create duplicates of the masternode, it will use the COIN_NAME parameter as name if a prof_name is not provided.\
			...
			\n  - ${YELLOW}dupnode iplist                                   ${NC}Shows all your configurated IPv4 and IPv6.\
			\n  - ${YELLOW}dupnode ipadd <ip> <netmask> [interface]         ${NC}Allows the system to recognize a new IPv4 or IPv6.\
			\n  - ${YELLOW}dupnode ipdel <ip> <netmask> [interface]         ${NC}Deletes an already recognized IPv4 or IPv6.\
			...
			\n  - ${YELLOW}dupnode list [prof_names...] [params...]         ${NC}Shows the amount of duplicated instances of every masternode, if a profile name/s are provided, it lists an extended info of the profile/s instances.\
			\n      ${YELLOW}[params...]${NC} list:\
...
function main() {
	...
	case "$1" in
		...
		"iplist")
			...
			;;
		"ipadd")
			exit_no_param "$3" "${YELLOW}dupnode ipadd <ip> <netmask> [interface]${NC} requires a IP, a netmask and a interface name (if there's more than 1)"
			ip_parse $2
			cmd_ipmod "add" $2 $3 $4
			;;
		"ipdel")
			exit_no_param "$3" "${YELLOW}dupnode ipdel <ip> <netmask> [interface]${NC} requires a IP, a netmask and a interface name (if there's more than 1)"
			ip_parse $2
			cmd_ipmod "del" $2 $3 $4
			;;
		"rpcchange")
